...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):        
        ...
        for i in range(300):
            ...
            virusPop[i].append(patient.update())
#        virusPop.append(virusAtTime)
    ...
    pylab.plot(range(0, 300), avgPop, label= legend)
    pylab.show()
    #print avgPop
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            if drug not in self.resistances or self.resistances[drug] == False:
                raise NoChildException
        ...
        if random.random() < self.maxBirthProb * (1 - popDensity):
            ...
            for res in self.resistances:
                if random.random() < self.mutProb:
                    childResistances[res] = not self.resistances[res]
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.viruses:
            for drug in drugResist:
                if drug not in virus.resistances or virus.resistances[drug] == False:
                    break
            else:
                total += 1
        return total
    ...
    def update(self):
        ...
        for virus in viruses:
            try:
                v = virus.reproduce(popDensity, self.drugs)
                #print 'v = %s' % v
                self.viruses.append(v)
            except NoChildException:
                pass
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    virusPop = [[] for n in range(300)]
    resVirusPop = [[] for n in range(300)]
    for trial in range(numTrials):        
        #virusAtTime = []
        ...
        for i in range(300):
            ...
            #virusAtTime.append(patient.getTotalPop())
            virusPop[i].append(patient.update())
            resVirusPop[i].append(patient.getResistPop(['guttagonol']))
            if i == 149:
...
#        virusPop.append(virusAtTime)
    ...
    legend = pylab.legend(loc = 'upper center')
    pylab.plot(range(0, 300), avgPop, label= legend)
    pylab.plot(range(0, 300), avgResPop, label= legend)
    pylab.show()
    #print avgResPop
