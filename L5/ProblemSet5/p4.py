...
def DFS(graph, start, end, maxDistOutdoors, maxTotalDist, path = [], path_list = [], totalDistOutdoors = 0, totalDist = 0, shortestPath = None):
    ...
    for node in graph.childrenOf(nstart):
        if node not in path: #avoid cycles
            ...
            if totalDist + dist <= maxTotalDist and (not shortestPath or totalDist + dist <= shortestPath):
                if totalDistOutdoors + outdoors <= maxDistOutdoors:
                    totalDistOutdoors += outdoors
                    totalDist += dist
                    print 'totalDist: ', totalDist
                    print 'Total Outdoors: ', totalDistOutdoors
                    DFS(graph, node, end, maxDistOutdoors, maxTotalDist, path, path_list, totalDistOutdoors, totalDist, shortestPath)
                else:
                    continue
            else:
                continue
...
def bruteForceSearch(digraph, start, end, maxTotalDist, maxDistOutdoors):    
    ...
    totalDist = 0
    shortestPath = None
    path_list = DFS(digraph, start, end, maxDistOutdoors, maxTotalDist, path, path_list, totalDistOutdoors, totalDist, shortestPath)
    print path_list
    if not path_list:
        raise ValueError
